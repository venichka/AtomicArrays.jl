...
function Hamiltonian(S::SpinCollection)
    ...
    for i=1:N
        if S.spins[i].delta != 0.
            ...
            H += S.spins[i].delta*embed(b,i,sigmap_)*embed(b,i,sigmam_)
        end
    end
    for i=1:N, j=1:N
        if i==j
...
"""
    quantum.Hamiltonian_eff(S)
Effective Hamiltonian (Omega_ij - i/2*Gamma_ij) of the given System.
"""
function Hamiltonian_eff(S::SpinCollection)
    spins = S.spins
    N = length(spins)
    b = basis(S)
    H = SparseOperator(b)
