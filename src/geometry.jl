...
"""
    geometry.rectangle_old(a, b; Nx=2, Ny=2)
Positions of spins on a rectangular lattice in the xy-plane.
The lattice starts at the origin and continues into positive x and y direction.
# Arguments
* `a`: Spin-spin distance in x-direction.
...
* `Ny=2`: Number of spins into y direction.
"""
rectangle_old(a::S, b::T; Nx::Int=2, Ny::Int=2) where {S<:Real,T<:Real} = vec([[i*a,zero(S),zero(S)]+[zero(T),j*b,zero(T)] for i=0:Nx-1, j=0:Ny-1])
...
The lattice starts at the origin and continues into positive x and y direction.
"""
square(a::Real; Nx::Int=2, Ny::Int=2) = rectangle_old(a, a; Nx=Nx, Ny=Ny)
