...
@everywhere begin
    ...
    a2_list = range(1.5e-1, 10e-1, NMAX)
    E_list = range(1e-3, 1.5e-2, NMAX)
    L_list = range(1.5e-1, 10e-1, NMAX)
    a1_list = range(1.5e-1, 10e-1, NMAX)
...
c12 = axs[1, 2].contourf(delt_list, a1_list, efficiency[opt_idx[1], opt_idx[2], :, :], cmap="bwr")
axs[1, 2].set_xlabel(L"\delta / \lambda_0")
axs[1, 2].set_ylabel(L"a_1/\lambda_0")
axs[1, 2].set_title("Objective (larger better)")

c13 = axs[1, 3].contourf(E_list, a1_list, efficiency[:, opt_idx[2], :, opt_idx[4]]', cmap="bwr")
axs[1, 3].set_xlabel(L"E_0")
axs[1, 3].set_ylabel(L"a_1/\lambda_0")
...
c22 = axs[2, 2].contourf(a1_list, L_list, efficiency[opt_idx[1], :, :, opt_idx[4]], cmap="bwr")
axs[2, 2].set_xlabel(L"a_1/\lambda_0")
...
#fig_1.savefig(PATH_FIGS*"obj4D_dimer_4x4_mpc_nit10.png", dpi=300)
...
save(PATH_DATA*"obj4D_lat_dimer_4x4_mpc.h5", data_dict_obj)
save(PATH_DATA*"fs4D_lat_dimer_4x4_mpc.h5", data_dict_sig)

data_dict_loaded = load(PATH_DATA*"obj4D_lat_dimer_4x4_mpc.h5")
