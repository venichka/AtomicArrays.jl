...
γ = -2 .* imag.(λ)
s_ind_max = sortperm(γ, rev=true)[2]
s_ind = sortperm(γ)[1]
ψ = states[s_ind]
...
x_xy, y_xy, I_xy = intensity_xy()
x_xz, z_xz, I_xz = intensity_xz()
...
function phases_ampl_lattice_plot()
    ...
    Axis(f[1, 1], title=L"Dark state$$",
        ...
        xticklabelsize=22,
        yticklabelsize=22)
    sc11 = scatter!(x_at[1:N÷2], y_at[1:N÷2], markersize = 30,
                    color = (abs.((ψ.data)[1:Nx*Ny])/
                    (findmax(abs.((ψ.data)[1:Nx*Ny]))[1])),
                    colormap=:BuPu_6)
    ...
    Axis(f[2, 1],
        ...
        xticklabelsize=22,
        yticklabelsize=22)
    sc21 = scatter!(x_at[1:N÷2], y_at[1:N÷2], markersize = 30, colorrange=(-pi, pi),
                    color = angle.(ψ.data[1:Nx*Ny]),
                    colormap=:rainbow1)
    ...
    Axis(f[1, 3], title=L"Bright state$$",
        ...
        xticklabelsize=22,
        yticklabelsize=22)
    sc13 = scatter!(x_at[1:N÷2], y_at[1:N÷2], markersize = 30,
                    color = (abs.((ψ_max.data)[1:Nx*Ny])/
                    (findmax(abs.((ψ_max.data)[1:Nx*Ny]))[1])),
                    colormap=:BuPu_6)
    ...
    Axis(f[2, 3],
        ...
        xticklabelsize=22,
        yticklabelsize=22)
    sc23 = scatter!(x_at[1:N÷2], y_at[1:N÷2], markersize = 30, colorrange=(-pi, pi),
                    color = angle.(ψ_max.data[1:Nx*Ny]),
                    colormap=:rainbow1)
...
function field_plot()
    GLMakie.activate!()
    f = Makie.Figure(resolution=(1200, 700))
    # Plot
    # X-Y
    Axis(f[1, 1], #title=L"Dark state$$",
        xlabel=L"x/\lambda_0",
    ...
    sc11 = scatter!(x_at[1:N÷2], y_at[1:N÷2], markersize = 10,
                    color = :black)
    Colorbar(f[2, 1], c11, label = L"$$Intensity", width = Relative(1),
             labelsize=28, ticklabelsize=18, vertical=false,
             ticks=([minimum(I_xy), maximum(I_xy)], ["0", "max"])
             )
    # X-Z
    Axis(f[1, 2], #title=L"Dark state$$",
        xlabel=L"x/\lambda_0",
        ylabel=L"z/\lambda_0",
        xlabelsize=28, ylabelsize=28,
        titlesize=30,
    ...
    #                 * "x" * string(Ny) * "_D" * ".pdf"), f) # here, you save your figure.
    return f
end
